...
Since Github hasn't added CORS headers to the git clone endpoint yet, we have to use a [proxy server](https://git-cors-proxy.now.sh/).
...
await git.clone({
    fs,
    dir,
    url: 'https://git-cors-proxy.now.sh/github.com/isomorphic-git/isomorphic-git',
    ref: 'master',
    singleBranch: true,
    depth: 10
});
